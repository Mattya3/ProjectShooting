...
void Battle::timer() {//メイン動作用の変数、毎回呼び出す
    short heroBulletBeforeSize = heroBullets.size();
    ...
    for(short i = 0; i < enemy.size(); i++) {//各敵を動作させる
    ...
    for(short i = 0; i < heroBullets.size(); i++) heroBullets.at(i).timer(enemyPoints, enemyLarge);
    for(short i = 0; i < enemyBullets.size(); i++) enemyBullets.at(i).timer(heroPoints, heroLarge);
...
void Battle::collision(short size) {//接触判定のメソッド
    ...
    for(short i = 0; i < dis.size(); i++) heroBullets.erase(heroBullets.begin() + dis.at(i));//自機とぶつかった自機の弾を消失
    dis = hero.collision(enemyBullets, enemyBullets.size());
    for(short i = 0; i < dis.size(); i++) enemyBullets.erase(enemyBullets.begin() + dis.at(i));//自機とぶつかった敵の弾を消失
    for(short j = 0; j < enemy.size(); j++) {//敵とぶつかった際の上記の処理
        dis = enemy.at(j).collision(heroBullets, heroBullets.size());
        for(short i = 0; i < dis.size(); i++) heroBullets.erase(heroBullets.begin() + dis.at(i));
    }
    for(short i = heroBullets.size() - 1; i >= 0; i--){//反射回数をオーバーした自機の弾を消失
        if(heroBullets.at(i).nonReflect()) heroBullets.erase(heroBullets.begin() + i);
    }
    for(short i = enemyBullets.size() - 1; i >= 0; i--){//反射回数をオーバーした敵の弾を消失
        if(enemyBullets.at(i).nonReflect()) enemyBullets.erase(enemyBullets.begin() + i);
    }
    for(short i = enemy.size() - 1; i >= 0; i--){
...
void Battle::loading(short stage) {//ステージ読み込みのメソッド
    ...
    while(getline(files, line)) {//ファイル全体を読み込む
        ...
        for(short i = 0; i < pattern; i++) {//動作パターンの数だけその敵に追加する
            getline(files, line);
